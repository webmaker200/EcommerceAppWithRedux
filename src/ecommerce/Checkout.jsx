...
import {addCourierAmount} from '../store/CartSlice'

function Checkout() {
    const totalAmount = useSelector((state)=> state.cart?.totalAmount || 0)
    const totalQuantity = useSelector((state)=> state.cart?.totalQuantity || 0)
    const dispatch = useDispatch()
    const totalTax = totalAmount * 0.15
    const [isChecked, setIsChecked] = useState(false)
    const [selectedPayment, setSelectedPayment] = useState("")
    const handleCourierAmount = (paymentMethod)=>{
        
      if(paymentMethod === "pay-on-delivery"){
        dispatch(addCourierAmount({amount : 15}))
      }else{
        dispatch(addCourierAmount({amount : -15}))
      }
    }
    ...
    const handlePaymentChange = (e)=>{
        ...
        if(selectedPayment === "pay-on-delivery" && paymentMethod !== "pay-on-delivery"){
            handleCourierAmount("")
        }
        setSelectedPayment( paymentMethod)
        if(paymentMethod === "pay-on-delivery"){
            handleCourierAmount(paymentMethod)
        }
      ...
      <div className="min-w-0 flex-1 space-y-8">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 ps-4 dark:border-gray-700 dark:bg-gray-800">
              <div className="flex items-start">
                <div className="flex h-5 items-center">
                  <input onChange={handlePaymentChange} id="pay-on-delivery"  type="radio" name="payment-method" className="h-4 w-4 border-gray-300 bg-white text-primary-600 focus:ring-2 focus:ring-primary-600" />
                </div>
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 ps-4 dark:border-gray-700 dark:bg-gray-800">
              <div className="flex items-start">
                <div className="flex h-5 items-center">
                  <input onChange={handlePaymentChange} id="paypal-2" aria-describedby="paypal-text" type="radio" name="payment-method" value="" className="h-4 w-4 border-gray-300 bg-white text-primary-600 focus:ring-2 focus:ring-primary-600 dark:border-gray-600 dark:bg-gray-700 dark:ring-offset-gray-800 dark:focus:ring-primary-600" />
                </div>
        ...
        <div>
          ...
          <div className="flex max-w-md items-center gap-4">
            ...
            <button type="button" className="flex items-center justify-center rounded-lg px-5 py-2.5 text-sm font-medium text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300">Apply</button>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 w-full space-y-6 sm:mt-8 lg:mt-0 lg:max-w-xs xl:max-w-md">
        <div className="flow-root">
          <div className="-my-3 divide-y divide-gray-200 dark:divide-gray-800">
            ...
            <dl className="flex items-center justify-between gap-4 py-3">
              <dt className="text-base font-normal text-gray-500 dark:text-gray-400">Tax (15%)</dt>
              <dd className="text-base font-medium text-gray-900 dark:text-white">${totalTax.toFixed(2)}</dd>
            </dl>

            <dl className="flex items-center justify-between gap-4 py-3">
              <dt className="text-base font-bold text-gray-900 dark:text-white">Total</dt>
              <dd className="text-base font-bold text-gray-900 dark:text-white">${(totalAmount+totalTax).toFixed(2)}</dd>
            </dl>
          </div>
        </div>

        <div className="space-y-3">
          <button type="submit" className="flex w-full items-center justify-center rounded-lg px-5 py-2.5 text-sm font-medium text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300">Proceed to Payment</button>
